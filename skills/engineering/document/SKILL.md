---
name: document
description: Creates a user-facing documentation page for a given topic in docs/<topic>.md. Accepts a topic name as the argument and writes a markdown guide from the perspective of someone using the project.
argument-hint: <topic>
---

# document — write a user-facing documentation page

You are running the `document` skill. Input: `$ARGUMENTS` — a topic name. It creates a user-facing documentation page for that topic and writes it to `docs/$ARGUMENTS.md`.

## What "user perspective" means

Write for someone running the project at the command line or scripting against it. They care about:
...
Do **not** discuss internals, source-file layout, or implementation details unless
they are directly visible to the user (e.g. environment variable names that come from
the source).

## Startup

1. Read `README.md` to understand the full CLI and interface surface.
2. Read any relevant spec files (e.g. `specs/briefing.md`, `specs/roadmap.md`,
   relevant epic files) to fill in details not yet in the README.
