...
# verify-epic — verify an epic's tests and AC coverage

You are running the `verify-epic` skill for epic **$ARGUMENTS**.

`$ARGUMENTS` is the epic ID (e.g. `EP-1`). It must be supplied — if it is missing or blank, stop and output:

> Usage: verify-epic &lt;epic-id&gt; — e.g. EP-1
...
> Test run failed: `<error summary>`. Fix the build before running the verify-epic skill.
