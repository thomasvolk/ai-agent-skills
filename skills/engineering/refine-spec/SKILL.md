...
# refine-spec — iteratively refine a technical specification

You are running the `refine-spec` skill for epic **$ARGUMENTS**.
...
> No PRD found for $ARGUMENTS. Run the `refine-epic` skill for $ARGUMENTS first to create and refine the PRD before generating a technical specification.
...
## Open Decisions
{High-impact choices awaiting user selection via checkbox. Check your preferred option, then re-run the refine-spec skill to reconcile.}
