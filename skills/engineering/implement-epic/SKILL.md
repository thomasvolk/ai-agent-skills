---
name: implement-epic
description: Implement an epic from its technical specification using TDD. Reads the spec, writes tests, implements requirements, runs tests, fixes issues until all pass, then delegates final QA to the verify-epic skill.
argument-hint: <epic-id>
---

# implement-epic — implement an epic from its technical specification using TDD

You are running the `implement-epic` skill for epic **$ARGUMENTS**.
...
> No PRD found for $ARGUMENTS. Run the `refine-epic` skill for $ARGUMENTS first.

If `specs/$ARGUMENTS-spec.md` does not exist, stop and output:

> No spec found for $ARGUMENTS. Run the `refine-spec` skill for $ARGUMENTS first.
...
## Step 5 — QA: invoke the verify-epic skill
...
Invoke the `verify-epic` skill for $ARGUMENTS.
...
**If verify-epic reports failing tests**, fix the code — do not modify tests to make them pass artificially — then invoke the `verify-epic` skill again. Repeat until no tests are failing.
